...
if 'yearly_hourly_results' not in st.session_state: st.session_state.yearly_hourly_results = None
...
def get_yearly_hourly_data(years):
    return sdl.load_summary_hourly_for_year(years)
        ...
        for i in range(num_rows):
            ...
            for j in range(cols_per_row):
                ...
                if year_index < num_years:
                    year = int(total_rentals_by_year.index[year_index])
...
with tab3:
    ...
    with st.container(border=True):
        cols = st.columns([3, 1])
        with cols[0]:
            selected_years_hour = st.multiselect("연도 선택 ", options=list(range(2020, 2026)), default=[2023, 2024], key="yearly_hourly_year_select")
        with cols[1]:
            st.write("")
            if st.button("📈 연도별 시간대 패턴 분석", use_container_width=True, key="yearly_hourly_run_button"):
                if not selected_years_hour:
                    st.error("분석을 위해 연도를 선택해야 합니다.", icon="🚨")
                    st.session_state.yearly_hourly_results = None
                else:
                    with st.spinner(f"선택하신 연도의 시간대별 평균 데이터를 분석 중입니다..."):
        ...
        if results_df.empty:
            ...
            for i, year in enumerate(sorted_years):
                with chart_cols[i]:
                    ...
                    st.bar_chart(hourly_df, x='hour', y='avg_total_rentals', color="#2ECC71") 
    else:
